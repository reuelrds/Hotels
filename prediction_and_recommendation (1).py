...
prediction_data = prediction_data.drop(columns=['Unnamed: 16', 'Unnamed: 17', 'Unnamed: 18',
...
def similarity(data,text, top_n=3):
        arr = vec.transform(data.clean_data).toarray()
        query = vec.transform([text]).toarray()
        list_ = [compare(arr[i],query) for i in range(data.shape[0])]
        data["dist"] = list_
...
def recommendation(location , text, top_n=3):
  ...
  text = text.lower()
  location_found = prediction_data[(prediction_data["Hotel Location"] == location)].reset_index(drop=True)
  if location_found.empty:
    print('Sorry we are unable to find hotels in {}'.format(location))
  else:
    location_found_positive = location_found[location_found.sentiment == 1].reset_index(drop=True)
    location_found_negative = location_found[location_found.sentiment == -1].reset_index(drop=True)
...
recommendation("vancouver, canada","business trip")


# In[24]:


recommendation("calabria, italy","business trip", 1)
